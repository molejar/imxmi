...
class DatSegUEV(DatSegBase):
    ...
    def init(self, smx_data):
        ...
        for key, val in smx_data.items():
            if not isinstance(key, str):
                raise InitErrorUEV("{}: Property name must be a string !".format(self.full_name))
            key = key.upper()
            if key == 'DESC':
                if not isinstance(val, str):
                    raise InitErrorUEV("{}/DESC: Value must be a string !".format(self.full_name))
                self.description = val
            elif key == 'FILE':
                if not isinstance(val, str):
                    raise InitErrorUEV("{}/FILE: Value must be a string !".format(self.full_name))
                self.path = val
            elif key == 'DATA':
                if not isinstance(val, str):
                    raise InitErrorUEV("{}/DATA: Value must be a string !".format(self.full_name))
                self._txt_data = val
            else:
                raise InitErrorUEV("{}: Not supported property name \"{}\"".format(self.full_name, key))

        if self.path is None and self._txt_data is None:
            raise InitErrorUEV("{} FILE or DATA property must be defined !".format(self.full_name))
